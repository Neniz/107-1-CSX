...
# sep表示資料的分隔符號,header是指資料是否有包含欄位名稱,stringsAsFactor是指是否將文字儲存為因素向量
#.csv(comma-separated values) 和預設sep參數(空格）不同，需改成sep = ","
...
subset(df, Height > 170 & ToeicGrade > 990, select = c(Name, ToeicGrade, Height))
